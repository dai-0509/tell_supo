                    ...
                    <form action="{{ route('customers.update', $customer) }}" method="POST" class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <select name="industry" 
                                    ...
                                    <option value="IT" {{ old('industry', $customer->industry) === 'IT' ? 'selected' : '' }}>IT</option>
                                    <option value="製造業" {{ old('industry', $customer->industry) === '製造業' ? 'selected' : '' }}>製造業</option>
                                    <option value="小売業" {{ old('industry', $customer->industry) === '小売業' ? 'selected' : '' }}>小売業</option>
                                    <option value="金融業" {{ old('industry', $customer->industry) === '金融業' ? 'selected' : '' }}>金融業</option>
                                    <option value="医療・福祉" {{ old('industry', $customer->industry) === '医療・福祉' ? 'selected' : '' }}>医療・福祉</option>
                                    <option value="教育" {{ old('industry', $customer->industry) === '教育' ? 'selected' : '' }}>教育</option>
                                    <option value="建設・不動産" {{ old('industry', $customer->industry) === '建設・不動産' ? 'selected' : '' }}>建設・不動産</option>
                                    <option value="運輸・物流" {{ old('industry', $customer->industry) === '運輸・物流' ? 'selected' : '' }}>運輸・物流</option>
                                    <option value="飲食・宿泊" {{ old('industry', $customer->industry) === '飲食・宿泊' ? 'selected' : '' }}>飲食・宿泊</option>
                                    <option value="士業・コンサル" {{ old('industry', $customer->industry) === '士業・コンサル' ? 'selected' : '' }}>士業・コンサル</option>
                                    <option value="その他" {{ old('industry', $customer->industry) === 'その他' ? 'selected' : '' }}>その他</option>
                                </select>
                                @error('industry')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <select name="temperature_rating" 
                                    ...
                                    <option value="">選択してください</option>
                                    <option value="A" {{ old('temperature_rating', $customer->temperature_rating) === 'A' ? 'selected' : '' }}>A (最高)</option>
                                    <option value="B" {{ old('temperature_rating', $customer->temperature_rating) === 'B' ? 'selected' : '' }}>B (高)</option>
                                    <option value="C" {{ old('temperature_rating', $customer->temperature_rating) === 'C' ? 'selected' : '' }}>C (中)</option>
                                    <option value="D" {{ old('temperature_rating', $customer->temperature_rating) === 'D' ? 'selected' : '' }}>D (低)</option>
                                    <option value="E" {{ old('temperature_rating', $customer->temperature_rating) === 'E' ? 'selected' : '' }}>E (最低)</option>
                                    <option value="F" {{ old('temperature_rating', $customer->temperature_rating) === 'F' ? 'selected' : '' }}>F (対象外)</option>
                                </select>
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="priority" 
                                    ...
                                    <option value="">選択してください</option>
                                    @for($i = 1; $i <= 5; $i++)
                                        <option value="{{ $i }}" {{ old('priority', $customer->priority) == $i ? 'selected' : '' }}>
                                            {{ $i }}{{ $i == 1 ? ' (最高)' : ($i == 5 ? ' (最低)' : '') }}
                                        </option>
                                    @endfor
                                </select>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="memo" 
                                      id="memo" 
                                      rows="4" 
                                      class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('memo') border-red-500 @enderror">{{ old('memo', $customer->memo) }}</textarea>
                            @error('memo')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
