...
<div class="p-6">
    ...
    <div class="mb-8">
        <div class="flex items-center space-x-4 mb-6">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900">顧客情報編集</h1>
                <p class="text-gray-600 mt-1">{{ $customer->company_name }}の情報を編集します</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="temperature_rating" 
                        ...
                        <option value="A" {{ old('temperature_rating', $customer->temperature_rating) == 'A' ? 'selected' : '' }}>A（最高 - 即決見込み）</option>
                        <option value="B" {{ old('temperature_rating', $customer->temperature_rating) == 'B' ? 'selected' : '' }}>B（高 - 強い関心）</option>
                        <option value="C" {{ old('temperature_rating', $customer->temperature_rating) == 'C' ? 'selected' : '' }}>C（中 - 検討中）</option>
                        <option value="D" {{ old('temperature_rating', $customer->temperature_rating) == 'D' ? 'selected' : '' }}>D（低 - 情報収集段階）</option>
                        <option value="E" {{ old('temperature_rating', $customer->temperature_rating) == 'E' ? 'selected' : '' }}>E（最低 - 関心薄）</option>
                        <option value="F" {{ old('temperature_rating', $customer->temperature_rating) == 'F' ? 'selected' : '' }}>F（要検討 - 再アプローチ必要）</option>
                    </select>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="status" 
                        ...
                        <option value="">選択してください</option>
                        <option value="new" {{ old('status', $customer->status) == 'new' ? 'selected' : '' }}>新規</option>
                        <option value="contacted" {{ old('status', $customer->status) == 'contacted' ? 'selected' : '' }}>連絡済み</option>
                        <option value="interested" {{ old('status', $customer->status) == 'interested' ? 'selected' : '' }}>興味あり</option>
                        <option value="not_interested" {{ old('status', $customer->status) == 'not_interested' ? 'selected' : '' }}>興味なし</option>
                        <option value="callback_scheduled" {{ old('status', $customer->status) == 'callback_scheduled' ? 'selected' : '' }}>コールバック予定</option>
                        <option value="closed" {{ old('status', $customer->status) == 'closed' ? 'selected' : '' }}>クローズ</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="priority" 
                        ...
                        <option value="">選択してください</option>
                        <option value="high" {{ old('priority', $customer->priority) == 'high' ? 'selected' : '' }}>高</option>
                        <option value="medium" {{ old('priority', $customer->priority) == 'medium' ? 'selected' : '' }}>中</option>
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-xl p-8 border border-white/20 shadow-lg">
            ...
            <div>
                ...
                <textarea name="memo" 
                          ...
                          class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent {{ $errors->has('memo') ? 'border-red-300' : '' }}">{{ old('memo', $customer->memo) }}</textarea>
                @error('memo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex justify-between">
            <div>
                <form method="POST" action="{{ route('customers.destroy', $customer) }}" class="inline" onsubmit="return confirm('この顧客を削除してもよろしいですか？削除した顧客情報は復元できません。')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
                            class="px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all duration-200 flex items-center space-x-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
